...
fn main() {
    ...
    let (duration, proof_size, verifier_duration, cycle_count) = bench_ecadd_precompile(n);
...
fn bench_ecadd_precompile(n: u32) -> (Duration, usize, Duration, usize) {
    ...
    let prove_info = prover.prove_with_opts(env, ECADD_ELF, &ProverOpts::succinct()).unwrap();
    ...
    receipt.verify(ECADD_ID).unwrap();
