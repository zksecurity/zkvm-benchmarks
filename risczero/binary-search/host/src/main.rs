...
fn main() {
    ...
    let (duration, proof_size, verifier_duration, cycle_count) = bench_binary_search(n);
...
fn bench_binary_search(n: usize) -> (Duration, usize, Duration, usize) {
    ...
    let prove_info = prover.prove(env, BINARY_SEARCH_ELF).unwrap();
