...
fn main() {
    ...
    let lengths = [4, 8];
    benchmark(benchmark_sha2, &lengths, "../../benchmark_outputs/sha2_stone.csv", "byte length");

}


fn benchmark_sha2(n: u32) -> (Duration, usize) {
    let program_path = "programs/sha256.cairo".to_string();
    let output_path = "programs/sha256_compiled.json".to_string();
    ...
    let output_file = format!("sha256_proof.json").to_string();
    let args = [
        "prove",
        "--cairo_version",
        "cairo0",
