...
fn main() {
    ...
    let (duration, proof_size, verifier_duration, cycle_count) = bench_sha3_chain_precompile(n);
...
fn bench_sha3_chain_precompile(iters: usize) -> (Duration, usize, Duration, usize) {
    let input = [5u8; 32];
    ...
    let prove_info = prover.prove_with_opts(env, SHA3_CHAIN_BENCH_ELF, &ProverOpts::succinct()).unwrap();
